export const themes = {
  light: {
    colors: {
      background: '#ffffff',
      title: '#000000',
      text: '#333333',
      yellow: '#F7DF1E',
      outstanding: '#000000',
      backgroundMenuMobile: '#FFFFFF',
      backgroundFooterTablet: '#FFFFFF',
      backgroundCardProject: '#FFFFFF',
      shadow: '0px 0px 20px rgba(0, 0, 0, 0.2)',
      gradientProjectTop: 'linear-gradient(180deg, rgba(0, 0, 0, 0.8) 0%, rgba(0, 0, 0, 0) 100%)',
      gradientProjectBottom: 'linear-gradient(180deg,rgba(0,0,0,0) 0%,rgba(0,0,0,0.6) 20%,rgba(0,0,0,0.9) 100%);',
      iconText: '#202020',
      error: '#FF4949',
      border: 'rgba(55, 53, 47, 0.20)',
      title404: 'rgba(0,0,0, .1)'
    }
  },
  dark: {
    colors: {
      background: '#202020',
      title: '#FFFFFF',
      text: '#AAAAAA',
      yellow: '#F7DF1E',
      outstanding: '#FFFFFF',
      backgroundMenuMobile: '#373C3F',
      backgroundFooterTablet: '#373C3F',
      backgroundCardProject: '#181818',
      shadow: 'none',
      gradientProjectTop: 'linear-gradient(180deg, rgba(40, 40, 40, 0.8) 0%, rgba(0, 0, 0, 0) 100%)',
      gradientProjectBottom: 'linear-gradient(180deg,rgba(0,0,0,0) 0%,rgba(40,40,40,0.6) 20%,rgba(40,40,40,0.9) 100%);',
      iconText: '#FFFFFF',
      error: '#FF4949',
      border: 'rgba(255, 255, 255, 0.20)',
      title404: 'rgba(255,255,255, .1)'
    }
  }
}
